...
@fig-4.2 (A) illustrates representative stress-strain behavior for several engineering materials obtained from specimens loaded at relatively slow strain rates.  Each material initially experiences a linear relationship between stress and strain, represented as a straight line on the stress/strain diagram. Some materials, such as cast iron fail at relatively small strains, whereas other materials, such as high density polyethylene (HDPE) often used for milk jugs, go to very large strains well beyond the axis limit.  Figure 4.2 (B)  shows the same data but plotted only up to a maximum strain of 5%.  This zoomed in version highlights several features: 1) many materials often exhibit a linear stress-strain behavior up to some limiting stress (or strain) level, known as the proportional limit; 2) Most of these materials exhibit a transition from this initial linear region to a more consistent plateau value (at what we will be defining as the yield strength); and that while the slope or stiffness (which we will define as the modulus) of materials can vary widely as one goes from metals to engineering plastics.  Figure 4.2 (C) shows the failure region of an aluminum alloy specimen tested to failure, illustrating a common failure mode.

![Results obtained from tensile tests. (A) Illustration of a typical stress-strain behavior for some common materials. Temperatures denote anneal temperatures. (B) Same data plotted up to 5% strain.  (C) Tensile test of an AlMgSi alloy. This is a ductile fracture type, as seen by the local necking and the cup and cone fracture surfaces.](images/v3.png){#fig-4.2 fig-align="center"}

Alhough not evident in Figure 4.2 because specimens were not unloaded prior to break, engineering materials often return to zero strain when stress is removed, provided the stress does not exceed a stress equal to the elastic limit.   Within this elastic region of material response, the object will return to its original dimensions when loads are removed. Although some materials, such as rubber, may be considered elastic even if their stress-strain behavior is nonlinear, many other engineering materials often exhibit linear stress-strain behavior within the elastic region. 

Materials such as glass, silicon wafers, and concrete tend to be quite brittle, breaking prior to significant permanent or plastic deformation. The behavior remains nearly linear until failure, an attribute of brittle materials. On the other hand, certain copper or aluminum alloys, mild steel, and many polymers are often quite ductile, meaning that there is considerable plastic deformation beyond some limiting value of stress or strain. Unless a material is perfectly brittle, the stress-strain relationship will cease to be linear, as evidenced by departures from linear elastic behavior on the stress/strain diagram. This nonlinear (often curved) portion is typically irrecoverable and is referred to as plastic deformation. If an object is loaded into the plastic region and the load is subsequently removed, any plastic deformation is permanent, and the object will not return to its original dimensions.
...
![Illustration of the proportional limit, the elastic limit, and the yield region of material response.](images/4.4%20new.png){#fig-4.4 fig-align="center" width="575"}

Other metrics are also often quantitatively evaluated from stress-strain diagrams, including the ultimate strength, $\sigma_{ult}$, the maximum stress achieved prior to failure and the strain at break, $\varepsilon_{break}$ which is a measure of the ultimate elongation capabilities of the material (though is often dependent on the size of the specimen’s gage length.
...
![Illustration of the initial linear elastic portions of the sress strain curce and slopes corresponding to (a) Young's modulus for uniaxial tension; (b) shear modulus for pure shear loading](images/CH4%20PNGs/4.6.png){#fig-4.5 fig-align="center" width="400"}
...
1.  $\varepsilon_T=\alpha \cdot \Delta T\\ \varepsilon_T=13 \times 10^{-6} /^\circ{F} *\left(-75^{\circ}{F}\right)\\ \varepsilon_T=-0.00098$

2.  $\varepsilon = \frac{\Delta L}{L},\;\;\Delta L=\varepsilon *L\\ \Delta L=-0.00098*24{~in}\\ \Delta L=-0.0235 {~in}$
:::
:::
:::
...
Materials are often tested under idealized conditions, such as uniaxial tension or shearing resulting from torsional loading. These tests are important because they provide material properties necessary for engineering design. Such simple stress states are less common in actual engineered components, where more complex, multiaxial stress states are present, as shown in @fig-4.6.

![Illustration of a general, multiaxial stress state, showing normal and shear stresses acting on the positive x-, y-, and z-planes. Note that equal and opposite stresses exist on the hidden faces of the element.](images/Chapter%204%20figure%20edits/figure%204.7.png){#fig-4.6 fig-align="center" width="526"}
...
Consider the x-y plane of the stress element in @fig-4.6. This 2D plane is shown in @fig-4.7. It experiences two normal stresses (σ~x~ and σ~y~) and two shear stresses (τ~xy~ and τ~yx~). τ~xy~ will cause the element to rotate counterclockwise while τ~yx~ will cause the element to rotate clockwise. For an element in equilibrium, these two stresses must be equal in order for these rotation effects to balance. Expanding this to the other planes we may say τ~xy~ = τ~yx~, τ~xz~ = τ~zx~, and τ~yz~ = τ~zy~.

![2D view of the x-y plane of a 3D stress element.](images/Chapter%204%20figure%20edits/figure%204.8.png){#fig-4.7 fig-align="center" width="392"}
...
Since the allowable stress in the wire is 200 MPa, the factor of safety can be determined from:
...
Figure 4.2: Results obtained from tensile tests. A: Mac McCord. 2024. CC BY-NC-SA. B: Sigmund. 2011. CC BY-SA 3.0. <https://commons.wikimedia.org/wiki/File:Al_tensile_test.jpg>.

Figure 4.3: Engineering materials are often loaded beyond yield to permanently deform the material, as in (A) the stamping process for an automotive panel or in (B) the crushing of a weld-bonded hat section used to absorb energy in an automobile accident. A: © Porsche/Schuler. 2019. Used under Fair Use. Retrieved from <https://www.autonews.com/suppliers/porsche-deal-lets-tool-supplier-shine>. B: © Ford Motor Company. Released under CC BY-NC-SA 4.0.

Figure 4.4: Illustration of the proportional limit, the elastic limit, and the yield region of material response. Kindred Grey. 2024. CC BY-NC-SA.

Figure 4.5: Illustration of the initial linear elastic portions of the sress strain curce and slopes corresponding to (a) Young’s modulus for uniaxial tension; (b) shear modulus for pure shear loading. Kindred Grey. 2024. CC BY-NC-SA.
...
Figure 4.6: Illustration of a general, multiaxial stress state, showing normal and shear stresses acting on the positive x-, y-, and z-planes. Kindred Grey. 2024. CC BY-NC-SA.

Figure 4.7: 2D view of the x-y plane of a 3D stress element. Kindred Grey. 2024. CC BY-NC-SA.
