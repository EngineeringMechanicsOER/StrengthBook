...
In @sec-14.2, the various sources of normal stress are reviewed and expanded on. They will be re-framed in terms of the general axes discussed in @sect-14.1 so as to clarify which other normal stresses they may combine with. In @sec-14.2.1 we will revisit bending stress to express the unsymmetric bending stress equation in terms of general cross-sectional axes. In @sec-14.2.3, eccentric loading, which is a specific instance of combined normal stress in which normal forces result in bending, will be considered.
...
Note that the determination of internal reactions for the general problems presented in this chapter will require the use of 3D equilibrium equations, particularly for determining bending moments and torsion. You may find it helpful to review @sec-1.4 to understand these calculations.
...
In previous chapters, individual sources of longitudinal normal stress were considered: axial loads (\@sec-axial-loading), bending loads (@sec-bending-loads), and pressure loads (@sec-thin-walled-pressure-vessels). With all three of these forms of loading present, we can express the generalized equation for total longitudinal stress as:
...
In @sec-9.3, an equation for bending stress in the x direction based on bending around the cross-sectional axes y and z was derived. That equation is:

$$
\sigma_x=-\frac{M_z y}{I_z}+\frac{M_y z}{I_y}
$$

The minus sign in front of the M~z~ term and the plus sign in front of the M~y~ term are part of the derived equation. When the correct signs for M~z~, M~y~, y, and z are inserted, the correct sign of the stress is obtained (positive for tensile and negative for compressive). In @sec-bending-loads, no other cross-sectional axes were considered so that focus on the general concept of bending stress could be more easily maintained.
...
$$
{\sigma_{(bending)}= \pm\left|\frac{M_V\ d_H}{I_V}\right| \pm\left|\frac{M_H\ d_V}{I_H}\right|}
$$ {#eq-14.1}
...
| *I~V~ = the moment of inertia about the vertical axis of the cross-section \[m^4^, in.^4^\]*
| *I~H~ = Area moment of inertia about the horizontal axis of the cross-section \[m^4^, in.^4^\]*
...
In solving problems, it will be helpful to note that M~H~ does not affect the normal stress of points directly on the H axis and M~V~ does not affect the normal stress of points directly on the V axis. This is evident from the fact that d~H~ =0 for a point on the V axis and d~V~ =0 for a point on the H axis.

![The distances d~H~ and d~V~ that would go in @eq-14.1 when calculating the bending stress for the example point P on the cross-section](images/chapter%2014%20figure%20updates%208.18.25/14.3.png){#fig-14.3 fig-align="center" width="279"}
...
1.  Draw the cross-section as viewed from the free end or cut end towards the origin. For example, in the case of the columns in @fig-14.4 and @fig-14.5, we would draw the top-down view of the cross-section.
...
![Bending moments drawn around vertical and horizontal axes shown with exaggerated moment arrows to assess tension versus compression.](images/chapter%2014%20figure%20edits/figure%2014.6.png){#fig-14.6 fig-align="center" width="425"}
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.1%20part%201.png){fig-align="center"}
...
\|M~x~\| = given = (376 kN mm)(1000 N/kN) = 376000 N mm
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.1%20part%203.png){fig-align="center"}

4\) Calculate:

\|M~z~\| = given = (204 kN mm)(1000 N/kN) = 204000 N mm
...
The sign for the first term is positive if the F~L~ is tensile (directed away from the cross-section) and negative if F~L~is compressive (pointed towards the cross-section).
...
In this case, the longitudinal axis is the y-axis. The vertical axis of the cross-section is the z-axis and the horizontal axis is the x-axis. With M~Rx~ = 0, we have:
...
\sigma_{yA}=\left|\frac{180,000{~N}}{(0.2{~m})(0.15{~m})}\right|- \left|\frac{(7200{~N}\cdot{m})(0.085{~m})}{\frac{1}{12}(0.15{~m})(0.2 {~m})^3}\right|+\left| \frac{(9000{~N}\cdot{m})(0.075{~m})}{\frac{1}{12}(0.2{~m})(0.15{~m})^3}\right| \\[20pt]\sigma_{y A}=11.9 MPa
$$
...
For the pressurized pipe loaded as shown, determine the total normal stress at point K. Noting that there would be no shear stress at point K for this loading, represent the general stress state of point K on a stress element. The pipe has an outside diameter of 200 mm and a wall thickness of 10 mm. The load P~z~ = 18 kN and the internal pressure in the pipe is 1500 kPa.

![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.4%20part%201.png){fig-align="center"}
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.4part%202.png){fig-align="center" width="336"}
...
M~Ry~ is not zero; however, point K is on the y-axis, so this point will not be affected by M~Ry~ (the x distance from the y-axis to point K on the cross-section is 0).
...
4.  Calculate, subbing values and signs from steps 1 and 3 into the equations from step 2.
...
As discussed in @sec-12.1, the sign of shear stress is based on the orientation of the shear stress arrows on the stress element that represents the stress state of the point of interest.To determine in which direction to draw a shear stress arrow on a stress element for a particular point, one must cut the body at one edge of the stress element and visualize the direction in which the shear load would act across the cut.

For example, in @fig-14.8 below, the stress element is shown to be in the L-H plane. When the cylinder is cut at the top of the stress element, we examine the bottom section of the cylinder and note that the reaction torque at the cut surface should be opposite in direction to that which is applied at the bottom surface. This results in a counterclockwise (from the top view perspective) reaction torque at the cut, which means that the top section tends to want to twist (and exert a force) from left to right across the top edge of the stress element.
...
![Observation of the direction of the shear load acting on the stress element indicates the sign of the shear stress.](images/chapter%2014%20figure%20updates%208.18.25/figure%2014.8.png){#fig-14.8 fig-align="center"}

### Shear Stress due to Transverse Shear Force {#sec-14.3.2}

In @sec-shear-loads, the equation $\tau=\frac{V Q}{I t}$ (@eq-10.2) was evaluated to find the shear stress due to a transverse shear force V that arises from a non-constant bending moment. In that chapter, only one direction of shear force was considered for each problem and the sign of the shear stress was not evaluated.
...
1.  The cut on the cross-section one makes to determine the first moment of area Q will be perpendicular to the direction of shear force direction (illustrated on the cross-sections in @fig-14.9).
2.  The area moment of inertia (I) will be based on the axis perpendicular to the shear force direction.
3.  The thickness (t), refers to the thickness of the solid part of the cross section that one cuts through to calculate Q (illustrated on the cross-sections in @fig-14.9).
...
$$
{\tau_{(transverse)}= \pm\left|\frac{V_H Q_V}{I_V t_V}\right| \pm\left|\frac{V_V Q_H}{I_H t_H}\right|}
$$ {#eq-14.3}
...
| *I~V~ = Area moment of inertia about the vertical centroidal axis of the cross-section \[m^4^, in.^4^\]*
| *I~H~ = Area moment of inertia about the horizontal centroidal axis of the cross-section \[m^4^, in.^4^\]*
| *t~V~ = Vertical thickness of the cross-section cu through \[m, in.\]*
| *t~H~ = Horizontal thickness of the cross-section cut through \[m, in.\]*

![Both directions of transverse shear force applied to a bar and the corresponding cuts on the cross-section used to find Q.](images/chapter%2014%20figure%20updates%208.18.25/figure%2014.9.png){#fig-14.9 fig-align="center"}
...
![Locations of zero transverse shear stress based on direction of shear force](images/chapter%2014%20figure%20edits/figure%2014.14.png){#fig-14.10 fig-align="center" width="461"}

The signs on the shear stress are determined similarly to the manner described in @sec-14.3.1 for torsion. The direction of the internal shear force is determined by cutting hte body at the point of interest, drawing the FBD of the cut section, and applying equilibrium. The resulting shear stress is considered to act in that same direction on the cut surface of the element.

For example, in @fig-14.9, point S is on the V-L plane. @fig-14.11 shows the element drawn from the side view, looking down the H axis. The shear force V~V~ acts downwards on the right side of the stress element. With the axes oriented such that the longitudinal axis is positive to the right, the right side of the stress element is the +L face. If the V axis of the element is positive upwards, than V~V~ is acting in the negative V direction on the +L face, indicating a [negative]{.underline} shear stress.

Similarly, point P is on the H-L plane. In @fig-14.11, the stress element is drawn from the top view (viewing down the V axis). The shear force V~H~ acts upwards on the right side of the stress element. If the right side is the +L face, and the H axis is positive upwards, then V~H~ acts in the +H direction on the +L face, indicating a positive shear stress.

![Stress elements for points S and P from @fig-14.9. The blue arrows correspond to the direction of the shear stress inducing shear force for the respective point on the cut face.](images/figure%2014.10.png){#fig14.11 fig-align="center" width="503"}
...
$$ {\tau= \pm\left|\frac{T \rho}{J}\right| \pm\left|\frac{V_H Q_V}{I_V t_V}\right| \pm\left|\frac{V_V Q_H}{I_H t_H}\right|} $$ {#eq-14.4}
...
| *Q~V~ = First moment of area about the vertical axis passing through the point of interest \[m^3^, in.^3^\]*
| *Q~H~ = First moment of area about the horizontal axis passing through the point of interest \[m^3^, in.^3^\]*
| *I~V~ = Area moment of inertia about the vertical centroidal axis of cross-section \[m^4^, in.^4^\]*
| *I~H~ = Area moment of inertia about the horizontal centroidal axis of cross-section \[m^4^, in.^4^\]*
| *t~V~ = Vertical thickness of the cross-section at cut made for Q~V~ \[m, in.\]*
| *t~H~ = Horizontal thickness of the cross-section at cut made for Q~H~ \[m, in.\]*
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.5%20part%201.png){fig-align="center"}
...
The torque is the moment about the longitudinal axis (M~Ry~). We find it by applying the moment equilibrium equation for the y axis. Note that on the FBD, the direction of M~RY~ is assumed to act counterclockwise around the y axis when viewing from the positive end to the negative (right to left).
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.6%20part%201.png){fig-align="center"}
...
While we can see by inspection R~z~ = 0, it is also not necessary to find since it only contributes to normal stress.

Similarly, the bending moments M~Rx~ and M~Ry~ are not necessary to find for this problem since they would also only contribute to the normal stress. However, this problem is re-visited in @exm-14.7 and those moments are found in that example.

Since both of the applied forces pass through the centroidal z-axis, M~Rz~ = T = 0.
...
As discussed in @sec-14.3.2, since point P is on the bottom edge of the cross-section (from the top view), it will not experience shear stress from V~x~. Similarly, point S is on the right edge of the cross-section (also from the top view), so it will not experience shear stress from V~y~.
...
V~y~ was found to act in the negative y direction, so it gets drawn directed to the left at the bottom of the P stress element. This results in a positive shear stress for point P.

$$\tau_{Pyz}=\frac{V_y\ Q_x}{I_x\ t_x}
$$

V~x~ was found to act in the positive x direction. so it gets drawn directed to the left at the bottom of the S stress element. This results in a negative shear stress for point S.

$$\tau_{Sxz}=-\frac{V_x\ Q_y}{I_y\ t_y}
$$

![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.6%20part%203.png){fig-align="center" width="204"}
...
$$
\begin{aligned}&Q_x=A'y'=\frac{1}{2}(160{~mm})(120{~mm})*\left(\frac{1}{2}\right)(80{~mm})=384000{~mm}^3=384 \times 10^{-6}{~m}^3\\[10pt]
&t_x=120\ mm = .12\ m
\end{aligned}
$$

![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.6%20part%205.png){fig-align="center" width="231"}
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.6%20part%206.png){fig-align="center" width="312"}
...
Re-examine @exm-14.6 to find the principal stresses and max/min shear stresses for point P on the post:

![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.7%20part%201.png){fig-align="center"}
...
Point P is on the x axis, so we know that it will not experience any bending stress from M~Rx~ .
...
With R~z~ = 0 and point P located on the x axis (see discussion in step 1), we can reduce the equation to:
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.7%20part%203.png){fig-align="center" width="241"}

4\) Calculate.

Recalling $I_y = 23.04 \times 10^{-6}{~m^4}$ from @exm-14.6, the normal stress at P is:
...
![](images/chapter%2014%20figure%20updates%208.18.25/example%2014.7%20part%204.png){fig-align="center" width="287"}

We can use the simplified version of the principal stress and the max/min shear equation (@eq-12.5); however, these equations were generally written for stresses in the xy plane:
...
In this problem, point P is on the yz-plane. Based on the discussion at the end of @sec-12.2.2., we can replace the x subscripts with y and the y subscripts with z, and calculate:
...
-   The bending moment contribution to normal stress comes from bending moments around the centroidal cross-sectional axes.
    ...
    -   Alternatively, one can use the visual aid described in @sec-14.1 to help determine if the bending results in tension or compression.
