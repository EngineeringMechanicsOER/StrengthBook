...
In @sec-4.7 we discussed the general state of stress. @fig-4.6 shows a 3D stress element subjected to the general state of stress – that is, 3 normal stresses (σ~x~, σ~y~, and σ~z)~ and 6 shear stresses τ~xy~, τ~xz~, τ~yx~ and τ~yz~, τ~zx~ and τ~zy~.

@fig-4.7 shows a stress element for the simplified case of plane stress – that is, stresses acting in a single plane. Under plane stress, the element experiences normal stresses σ~x~ and σ~y~, and shear stress τ~xy~ = τ~xz~.
...
& F_t=\sigma x^* d A x^* \sin (\theta)=\sigma x^* d A^{\prime} \cos (\theta) \sin (\theta)
\end{aligned}
$$
...
$$
\begin{aligned}
& F_n=-\tau_{xy}{ }^* d A x^* \sin (\theta)=-\tau x y^* d A^{\prime} \cos (\theta) \sin (\theta) \\
& F_t=-\tau_{xy}{ }^* d A x^* \cos (\theta)=-\tau x y^* d A^{\prime} \cos (\theta) \cos (\theta)
\end{aligned}
$$
...
$$
\begin{aligned}
& F_n=-\tau_{xy} * d A x^* \cos (\theta)=-\tau x y^* d^{\prime} \sin (\theta) \cos (\theta) \\
& F_t=\tau_{xy}{ }^* d A y^* \sin (\theta)=\tau_{x y}^* d A^{\prime} \sin (\theta) \sin (\theta)
\end{aligned}
$$
...
![Draw a sketch of the x-axis and the normal axis to ensure you use the correct magnitude and sign for θ in the stress transformation equations.](images/12.5.png){#fig-12.5 fig-align="center" width="418"}
...
\begin{aligned}
& \sigma_n=\frac{\sigma_x+\sigma_y}{2}+\frac{\sigma_x-\sigma_y}{2} \cos (2 \theta)+\tau_{x y} \sin (2 \theta) \\
& \sigma_n=\frac{-1700{~psi}+800{~psi}}{2}+\frac{-1700{~psi}-800{~psi}}{2} \cos(2*54^\circ)+790{~psi}* \sin(2*54^\circ) \\
& \sigma_n= 687.6{~psi} \\
& \tau_{n t}=-\frac{\sigma_x-\sigma_y}{2} \sin (2 \theta)+\tau_{x y} \cos (2 \theta) \\
& \tau_{n_t}=-\frac{-1700{~psi}-800{~psi}}{2} \sin (2*54^\circ)+790{~psi}*\cos (2*54^\circ) \\
& \tau_{n_t}=944.7{~psi}
\end{aligned}
$$
...
1.  Find the principal planes using\
    ...
    \begin{aligned}
    & \operatorname{tan}\left(2 \theta_p\right)=\frac{2 \tau_{x y}}{\left(\sigma_x-\sigma_y\right)} \\
    & \tan(2\theta_p)=\frac{2*-25{~MPa}}{-80{~MPa}-50{~MPa}} \\
    & \tan(2\theta_p)=0.3846 \\
    & 2\theta_p=\tan^{-1}(0.3846) \\
    &\theta_p = \frac{21.04^\circ}{2}=10.52^\circ\\
    \end{aligned}
    ```
...
2.  Subbing θ~p~ = 10.52° into:\
    ...
    ```{=tex}
    \begin{aligned}
    & \sigma_{n_1}=\frac{-80{~MPa}+50{~MPa}}{2}+\frac{-80{~MPa}-50{~MPa}}{2}\cos(2*10.52^\circ)-25{~MPa}*\sin(2*10.52^\circ) \\
    & \sigma_{n_1}=-84.65{~MPa} \\
    \end{aligned}
    ```
    and subbing θ~p~ = 100.52° in gives

    ```{=tex}
    \begin{aligned}
    & \sigma_{n_2}=\frac{-80{~MPa}+50{~MPa}}{2}+\frac{-80{~MPa}-50{~MPa}}{2}\cos(2*100.52^\circ)-25{~MPa}*\sin(2*100.52^\circ) \\
    & \sigma_{n_2}=54.65{~MPa}
    \end{aligned}
    ```
    Since 54.7 MPa is the most positive of the two principal stresses, σ~1~ = 54.7 MPa and σ~2~ = -84.7 MPa.

    The 54.7 MPa would go on the rotated x-face and the -85.7 MPa would go on the rotated y-face of the stress element rotated counterclockwise 100.52°.\
    \
    Conversely, if the stress element is rotated counterclockwise by 10.52°, the stress on the rotated x-face would be -84.7 MPa and the stress on the rotated y-face would be 54.6 MPa.

    Drawing the stress element rotated to the principal planes:

    ![](images/CH12%20figures/example%2012.3%20part%202.png){fig-align="center" width="245"}
...
4.  Subbing θ~s~ = 55.5° into:\
    ...
    Since this is a positive value, we know that this is the max in-plane shear stress and that θs = -34.5° should give τnt = -69.6 MPa = τ~min~ (in-plane).

    So if we rotate the stress element counterclockwise by 55.5°, we would draw a positive shear stress and if we rotate it clockwise 34.5°, we would draw a negative shear stress.
...
![Mohr's circle](images/CH12%20figures/figure%2012.11%20part%201.png){#fig-12.11a fig-align="center" width="468"}
...
![3D stress element with stress acting in only 2 or the 3 principal directions.](images/CH12%20figures/figure%2012.12.png){#fig-12.12 fig-align="center" width="564"}
...
More generally, given principal stresses σ~1~, σ~2~, and σ~3~ = 0, the aboslute maximum shear stress can be determined from:
...
3.  Connect the two points on the circle and identify the center.\
    ...
    ![](images/CH12%20figures/example%2012.4%20part%204.png){fig-align="center" width="444"}
...
4.  The radius of the circle is:
    ...
    &R=\sqrt{(-60{~MPa}-(-22.5{~MPa}))^2+(30{~MPa})^2}\\
    &R=48.02{~MPa}
    \end{aligned}
    $$

5.  We can now find σ<sub>1</sub> by finding the distance from the origin to the rightmost point of the circle on the σ axis. Since C is negative, we can find distance to be
    ...
    We can find σ<sub>2</sub> by finding the distance from the origin to the leftmost point of the circle on the σ axis. This distance can be expressed as
...
7.  The principal planes can be found as:

    $$
    \begin{aligned}
    &\tan(2\theta_p)=\frac{\sigma_x-C}{\tau_{xy}}\\
    &\tan(2\theta_p)=\frac{-60{~MPa}-(-22.5{~MPa})}{30{~MPa}}\\
    &\tan(2\theta_p)=-1.25\\
    &2\theta_p=\tan^{-1}(-1.25)\\
    &\theta_p=\frac{-51.34^\circ}{2}\\
    &\theta_p=-25.67^\circ
    \end{aligned}
    $$
    ...
    The other solution to the equation is that angle plus 90°, which in this case yields θ~p~ = 64.33°. This is the angle of rotation for which σ<sub>1</sub> would appear on the rotated x-face. The positive angle indicates this would be a counterclockwise rotation.

    ![](images/CH12%20figures/example%2012.4%20part%205.png){fig-align="center" width="301"}

8.  With the θp angles known, we can see that the 2θ~s~ = 90 - 2θ~p~, or

    $$
    \begin{aligned}
    &2\theta_s=90-2\theta_p\\
    &2\theta_{s_1}=90-2*-25.67^\circ\\
    &\theta_{s_1}=\frac{141.3^\circ}{2}\\
    &\theta_{s_1}=70.65^\circ\\
    &2\theta_{s_2}=90-2*64.33^\circ\\
    &\theta_{s_2}=\frac{-38.66^\circ}{2}\\
    &\theta_{s_2}=-19.33^\circ
    \end{aligned}
    $$

    In this case, a counterclockwise rotation of the x-point would bring it to the negative side of the vertical axis which corresponds to τmax, so we can say that θ~s~ = 70.65° is the plane of max in-plane shear stress.

    ![](images/CH12%20figures/example%2012.4%20part%206.png){fig-align="center" width="473"}
...
9.  Now we can use Mohr’s Circle to find the stress state on the plane where the stress element is rotated clockwise 40°. The circle is redrawn below for convenience.
    ...
    Recalling that rotations around the circle are twice the actual rotation, we will draw in a point (denoted as x’) that represents an 80 degree rotation clockwise from the x-point. Since we know from step 7 that the x-point is rotated 2\*25.67° below σ~2~, we also know that x’ is rotated 80 – 51.34 = 28.66° above σ~2~.

    ![](images/CH12%20figures/example%2012.4%20part%207.png){fig-align="center" width="265"}
    ...
    $$
    \begin{aligned}
    &\tau_{x^\prime y^\prime}=-R\sin(28.66^\circ)\\
    &\tau_{x^\prime y^\prime}=-48.08{~MPa}*\sin(28.66^\circ)\\
    &\tau_{x^\prime y^\prime}=-23.06{~MPa}
    \end{aligned}
    $$and
    ...
    &\sigma_{x^\prime}=-22.5{~MPa}-48.02{~MPa}*\cos(28.66^\circ)\\
    &\sigma_{x^\prime}=-64.64{~MPa}
    \end{aligned}
    $$
    ...
    $$
    \begin{aligned}
    &\sigma_{y^\prime}=C+R\cos(28.66^\circ)\\
    &\sigma_{y^\prime}=-22.5{~MPa}+48.02{~MPa}*\cos(28.66^\circ)\\
    &\sigma_{y^\prime}=19.64{~MPa}
    \end{aligned}
    $$

    The rotated stress element can be drawn:

    ![](images/CH12%20figures/example%2012.4%20part%208.png){fig-align="center" width="323"}

**σ~1~ = 25.5 MPa on the principal plane θ~p~ = 64.3°**

**σ~2~ = -70.5 MPa on the principal plane θ~p~ = -25.7°**

**τ~max~ (in-plane) = 48.0 MPa on the plane θ~s~ = 70.7°**
