...
As we found in previous chapters, internal forces and moments and section properties are crucial to calculating stresses and deflections. The same is true for finding stresses and deflections in beams. This chapter describes calculating bending stresses in beams, such as those in @fig-9.1, with considerations for unsymmetric bending and beam design.

![Looking up at bridge beams used to support a landscaped bicycle and pedestrian bridge in Seattle.](images/CH9%20PNGs/Figure%209.2.jpg){#fig-9.1 fig-align="center" width="470"}

In @sec-9.1 we'll derive an equation for calculating bending stress. In @sec-9.2 we'll discuss how to design beams to ensure they do not fail due to bending stress. In @sec-9.3 we'll extend our analysis to include cases where the bending moment acts not only about the horizontal axis of the beam, but has components about both the horizontal and vertical axes.
...
This section will discuss bending behavior of straight, symmetric, homogeneous beams. This section will be limited to beams that have a cross section that is symmetric with an axis and the bending moment is around an axis that is perpendicular to the axis of symmetry. Examine the initially unaltered beam depicted in @fig-9.2, characterized by a rectangular cross-section, and annotated with both horizontal and vertical grid lines.

![An unloaded beam with a rectangular cross-section.](images/CH9%20PNGs/Figure%209.3.png){#fig-9.2 fig-align="center" width="278"}

When a bending moment is applied, horizontal lines curve, while vertical lines remain straight but rotate, as illustrated in @fig-9.3. The material stretches at the bottom and compresses at the top, creating a **neutral surface** in between where fibers experience no length change. On a cross-section, this surface appears as the **neutral axis**.

![The deformation of the beam in @fig-9.2 when subjected to a symmetric bending moment.](images/chapter%209%20figure%20updates/9.3%20new.png){#fig-9.3 fig-align="center" width="269"}

We can make a similar observation when a kids toy undergoes bending as shown in @fig-9.4. This toy is subjected to a bending moment in the opposite direction of the beam in @fig-9.3, this will switch the behavior of the top and the bottom surfaces. Notice in @fig-9.4 the fibers at the top are pulled apart while the bottom surface fibers are pushed together.

![The distortion of a kid's toy when undergoing bending.](images/chapter%209%20figure%20updates/9.4.jpg){#fig-9.4 fig-align="center" width="336"}
...
![Column failure due to bending.](images/CH9%20PNGs/Figure%209.6.jpg){#fig-9.5 fig-align="center" width="407"}
...
![Element in beam bending.](images/chapter%209%20figure%20updates/9.6%20new.png){#fig-9.6 fig-align="center" width="334"}

To understand bending stress in a beam subjected to arbitrary loads, examine a small element extracted from the beam in @fig-9.6. The derivation of the bending strain equation remains unaffected by the beam type or specific loads. Remember the fundamental definition of normal strain:
...
Distance y is measured relative to the neutral surface and is positive above the neutral surface and negative below. Before bending, line AB is the same length at all y-values. During bending, line A’B’ shortens above the neutral surface and lengthens below it. By definition, the length of the neutral surface doesn't change and remains the same as length AB. We can describe the lengths AB and A'B' using the radius of curvature (ρ) and the differential angle (dθ) shown in @fig-9.6.
...
This relationship shows us that the longitudinal strain, ε, varies linearly with the distance, y, from the neutral surface. The maximum stress occurs at the outermost fibers, extreme top and bottom of the section. We call this maximum distance from the neutral surface, c. We can now write a relationship for the maximum absolute value of the strain, ε~max~.
...
![Bending stress variation.](images/chapter%209%20figure%20updates/9.7%20new.png){#fig-9.7 fig-align="center" width="429"}
...
| *where*
| *σ~max~ = Maximum bending stress in the beam, note that a complete description includes magnitude, units, tension, or compression \[Pa, psi\]*
...
*I = Area moment of inertia about the neutral axis \[m^4^, in.^4^\]*
...
Similar to @sec-stress, when reporting normal stress you include if that stress is in tension or compression. During the derivation, we used a beam that was bending concave upwards. In @sec-beams we indicated that this type of moment was considered positive. This leads to compression above the neutral axis and tension below. The inverse is true when the beam is subjected to negative bending moment as shown in @fig-9.8.

![Bending stress sign convention.](images/CH9%20PNGs/Figure%209.9.png){#fig-9.8 fig-align="center" width="499"}

@exm-9.2 and @exm-9.3 demonstrate the calculation of bending stresses at varrious points on a cross-section.
...
![](images/chapter%209%20figure%20updates/example%209.1%20new%20part%201.png){fig-align="center" width="243"}

::: {.callout-tip collapse="true" icon="false"}
## Solution

Since we are asked to find the bending stress, let’s look at the bending stress formula, equation 9.2.

$$
{\sigma=-\frac{M y}{I}}
$$

We first start with the internal moment, this is a given in the problem statement to be +30 KN\*m. Positive moment indicates that the beam is bending concave up, compression above the neutral axis and tension below.

![](images/chapter%209%20figure%20updates/example%209.1%20new%20part%202.png){fig-align="center" width="269"}

The value of *y* is the perpendicular distance from the neutral axis to the point of interest. In this problem we have three points of interests so we will have three different y values for this problem—one for the top, bottom, and point A.

We next need to determine the beam properties—centroid and moment of inertia. Since the cross section is a rectangle, the centroid is located in at the midpoint of the height of the beam. We can then use the formula for moment of inertia for a rectangle from Appendix E.

![](images/chapter%209%20figure%20updates/example%209.1%20new%20part%203.png){fig-align="center" width="273"}
...
Now we can calculate stress at each of the three points requested. We will start with the top:

$$
\sigma_{top}=\frac{(30*10^3Nm)(0.15m)}{1.8*10^{-4}*m^4}=25 MPa
$$

For a complete answer we need to include magnitude, units, and tension or compression. The magnitude is 25, the units will be MPa. To determine the tension or compression we will look at the sign of the internal moment and where along the section we are calculating the bending stress. In our example the moment is positive, and we calculate the stress at the top of the section our answer will be in compression.

![](images/chapter%209%20figure%20updates/example%209.1%20new%20part%202.png){fig-align="center" width="265"}
...
We will now calculate the normal stress at the bottom of the beam.  In this case all of the values (M, y, and I) are the same so the magnitude of the stress is the same (25MPa). The piece that changes is the type of stress (tension or compressions). Since the moment is positive and we are calculating the stress at the bottom of the beam, the stress will be in tension.

$$
\sigma_{BTM}=25MPa(T)
$$

Finally, we will calculate the stress at point A, which is located 100mm from the top of the section. The y value measures from the neutral axis (centroid). So we will use y = 50 mm.

$$
\sigma_{A}=\frac{(30*10^3Nm)(0.05m)}{1.8*10^{-4}*m^4}=8.33 MPa
$$

Similarly, we need to evaluate if the stress is in tension or compression.  Since point A is above the neutral axis and the moment is positive the stress at point A will be in compression.
...
We next need to determine the beam properties—centroid and moment of inertia. Since this is not a standard shape and the shape is not symmetric in the y direction, we will need to calculate this by hand. We covered this shape in detail in @exm-8.1 and @exm-8.4.
...
The first step is to find the internal forces in the beam so that we can find the maximum positive and negative moments. The best way to do this is to draw the moment diagram. We did this in detail in @exm-7.3.
...
We next need to determine the beam's geometric properties—centroid and moment of inertia. Since this is not a standard shape and the shape is not symmetric in the y direction, we will need to calculate this by hand. We covered this shape in detail in @exm-8.2 and @exm-8.5.
...
![The end of the prestressed concrete beam showing the prestressing steel and rebar](images/figure%209.10%20(needs%20replacement).jpg){#fig-9.9 fig-align="center" width="493"}
...
2.  Determine the section properties of the beam. This will require knowing the location of the **centroid**, **neutral axis**, and the **moment of inertia**. (Review of this is in @sec-geometric-properties of this text or a table listing values of I for selected common shapes is in @sec-centroids-moments-inertia.)
...
| *where*
| *Smin = Minimum required section modulus for the beam to resist the bending stress \[m^3^, in.^3^\]*
...
To prevent failure from bending stress, the beam's section modulus must exceed S~min~. Rolled steel beams have complex, standardized cross-sections to simplify design and manufacturing. Engineers select a suitable beam from these standard sizes. Often there are many criteria that you will need to consider. In this section we will use cost to determine the shape. Steel beams are priced by their weight, the higher the weight the more expensive. To choose the most economical shape, we will choose the shape with the smallest weight (smallest cross-sectional area).

There are basic structural steel shapes that have standardized cross sectional dimensions. W shapes (standing for Wide Flange) are often used for beam design as they have an efficient cross section. @fig-9.10 is a sample of the beam table in @sec-geometric-properties-of-standard-beams.

![Portion of the W Section table from @sec-geometric-properties-of-standard-beams](images/Updated%20appendices%20figures/figure%209.10.png){#fig-9.10 fig-align="center"}

Each beam has a designation that gives us quick information about the section, see @fig-9.11.

![Description of the Shape designation in U.S. Customary Units.](images/CH9%20PNGs/Figure%209.12.png){#fig-9.11 fig-align="center" width="368"}

Notice in the sample beam section table in @fig-9.10 they are all W shapes and grouped by approximate height (first number). Within each height grouping the shapes are arranged from lightest to heaviest (second number) when going from the bottom to top of the height grouping. For each standard shape in this table the cross-sectional area, depth (height), flange and web dimensions, moment of inertia in the x and y directions, section modulus, in the x and y direction, and the radius of gyration in the x and y directions (this is a geometric property related to buckling, we will use this in @sec-columns of this text) are given.

In certain instances, the selection of a section might be constrained by factors such as the permissible depth of the cross-section or the allowable deflection of the beam. This discussion is limited to materials that behave the same in tension and compression. If this is not the case (such as when you use concrete), then you may need to check multiple points along the length of the beam. Additionally, if the section is not symmetric about the neutral axis, the maximum tensile and compressive stresses may not occur at the place on the beam. Lastly, this procedure only considers bending stresses. Although bending stresses do control the design of most beams, there are instances where shear or deflection will control. We will discuss this more in @sec-beam-deflection in this text.
...
![](images/chapter%209%20figure%20updates/example%209.3%20new%20part%201.png){fig-align="center" width="361"}
...
![](images/chapter%209%20figure%20updates/example%209.3%20new%20part%201.png){fig-align="center" width="347"}

![](images/chapter%209%20figure%20updates/example%209.3%20new%20part%202.png){fig-align="center" width="348"}

![](images/chapter%209%20figure%20updates/example%209.3%20new%20part%203.png){fig-align="center" width="347"}
...
Now that we know the minimum section modulus needed for this beam, we will go to the standard beam table in @sec-geometric-properties-of-standard-beams. Focusing on the S~x~ column, we will start at the bottom of the table and choose one shape from each height section that is applicable.
...
2.  Determine the maximum absolute value of the bending moment \|M~max~\| **i**n the beam by drawing the shear force and bending moment diagrams. (Review of this is in @sec-beams of this text).
...
![Moment arbitrarily applied to a rectangular cross section.](images/CH9%20PNGs/Figure%209.13.png){#fig-9.12 fig-align="center" width="524"}

To systematize this process, envision a beam with a rectangular cross-section subjected to a moment, **M**, as depicted in @fig-9.12 (A). Here, **M** forms an angle, θ, with the maximum principal z-axis, which is the axis of maximum moment of inertia for the cross-section. It is assumed that θ is positive when directed from the positive z-axis towards the positive y-axis. We can resolve **M** into y and z components:
...
| *M~z~, M~y~ = Internal bending moment components about the z- and y-axes \[N·m, lb·in.\]*
| *y, z = Distance in the y- and z-directions from the centroid to the point of interest \[m, in.\]*

*I~z~, I~y~ = Area moment of inertia about the centroidal z- and y-axes \[m^4^, in.^4^\]*
...
@exm-9.5 and @exm-9.6 demonstrate how to calculate bendign stress at various points on a cross-section when the moment is arbitrarily applied.
...
![](images/chapter%209%20figure%20updates/example%209.6%20part%201.png){fig-align="center" width="404"}
...
$$
\sigma_A=-\frac{-169.0{~kip}\cdot{in.} *-4{~in.}}{640{~in.}^4}+\frac{362.5{~kip}\cdot{in.} * 7.5{~in.}}{2250{~in.}^4}\\ \sigma_A=-1.06{~ksi}+1.21{~ksi}=0.15{~ksi}=0.15{~ksi}~(T)
$$

B:

$$
\sigma_B=-\frac{-169.0{~kip}\cdot{in.} * 4{~in.}}{640{~in.}^4}+\frac{362.5{~kip}\cdot{in.} * 7.5{~in.}}{2250{~in.}^4}\\ \sigma_B=1.05{~ksi}+1.21{~ksi}=2.26{~ksi}=2.26{~ksi }~(T)
$$

C:

$$
\sigma_C=-\frac{-169.0{~kip}\cdot{in.} * 4{~in.}}{640{~in.}^4}+\frac{362.5{~kip}\cdot{in.} *-7.5{~in.}}{2250{~in.}^4}\\ \sigma_C=1.06{~ksi}-1.21{~ksi}=-0.15{~ksi}=0.15{~ksi}~(C)
$$

D:

$$
\sigma_D=-\frac{-169.0{~kip}\cdot{in.} *-4{~in.}}{640{~in.^4}}+\frac{362.5{~kip}\cdot{in.} *-7.5{~in.}}{2250{~in.}^4}\\ \sigma_D=-1.05{~ksi}-1.21{~ksi}=-2.26{~ksi}=2.26{~ksi}~(C)
$$
...
![](images/chapter%209%20figure%20updates/example%209.6%20part%202.png){fig-align="center" width="384"}
...
The dimensions of the W14 x 43 beam can be found in @sec-geometric-properties-of-standard-beams. We can also look up the area moment of inertia about both axes.
